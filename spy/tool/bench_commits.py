...
def run_benchmarks_in_ref(
    ...
    *,
    ...
    with temp_worktree() as tmp_dir:
        run_in_dir(["git", "checkout", ref.ref], cwd=tmp_dir, verbose=verbose)
        ...
        output(f"\nRunning tests in {tmp_dir}")

        results = []
        for bench in benchlist:
            output(f"\nRunning benchmark {bench}")
            for i in range(num_runs):
                output(f"Interpreter run {i + 1: >3}: ", end="")
...
def print_data(all_results: list[SingleResult]) -> None:
    ...
    for execution_kind in kinds:
        ...
        refs_in_results = list({sr.ref for sr in k_results})
        benchmarks = {sr.benchfile for sr in k_results}

        headers = ["Benchmark", *[ref.header_name for ref in refs_in_results]]
        table = []

        for bench in benchmarks:
            b_results = [sr for sr in k_results if sr.benchfile == bench]
            line: list[Path | str] = [
                (bench.name if isinstance(bench, Path) else bench)
            ]
            for ref in refs_in_results:
                ref_results = [sr for sr in b_results if sr.ref == ref]
...
def main():
    ...
    refs = [Ref(ref, name) for ref, name in zip(args.refs, args.ref_names)]
