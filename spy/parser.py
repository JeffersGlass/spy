...
class Parser:
    ...
    _current_module: spy.ast.Module | None = None
    ...
    def unsupported(self, node: py_ast.AST, reason: Optional[str] = None) -> NoReturn:
        ...
        """
        if reason is None:
            reason = f"{node} is of type {node.__class__.__name__}"
    ...
    def create_global_func_for_List(self, py_node: py_ast.List) -> spy.ast.Call:
        ...
        body = [
            py_ast.Assign(
                _loc = py_node.loc,
                targets=[py_ast.Name(_loc=py_node.loc, id=list_name, ctx=py_ast.Store(), spy_varkind=None)],
                value=py_ast.Call(
                    _loc=py_node.loc,
                    func = py_ast.Name(
                        _loc=py_node.loc,
