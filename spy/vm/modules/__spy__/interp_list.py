...
class W_InterpList(W_BaseInterpList, Generic[T]):
    ...
    def w_LEN(vm: "SPyVM", wam_list: W_MetaArg) -> W_OpSpec:
        ...
        @vm.register_builtin_func(w_listtype.fqn)
        def w_len(vm: "SPyVM", w_lst: LIST) -> W_I32:
            return vm.wrap(len(w_lst.items_w))
    ...
    def w_APPEND(vm: "SPyVM", wam_list: W_MetaArg, wam_item: W_MetaArg) -> W_OpSpec:
        ...
        @vm.register_builtin_func(w_listtype.fqn)
        def w_append(vm: "SPyVM", w_lst: LIST, obj: T) -> W_Str:
            w_lst.items_w.append(obj)
            return obj

        return W_OpSpec(w_append, [wam_list, wam_item])
