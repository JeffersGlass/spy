...
def xfail_selected_params(request):
    ...
    if (src_name, mode) in allowed_failures:
        request.node.add_marker(
            pytest.mark.xfail(reason=allowed_failures[(src_name, mode)])
        )


@pytest.mark.parametrize(
    "src_name", ["generic", "list_literal", "list_add", "list_mul", "tuple_literal"]
)
...
async def test_list_j(src_name, mode, xfail_selected_params):
    ...
    modname = (
        args.filename.stem
    )  # | These lines are critical or we can't analyze imports later
    ...
    debug(f"After parse_all() importer.mods=")
    for k, v in importer.mods.items():
        debug(f"\t{str(k): <25}{shorten(str(v), 50): <55}{type(v)}")
    # TODO What can we assert here?
